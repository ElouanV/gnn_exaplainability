...
class PlotUtils(object):
    ...
    def plot(self, graph, nodelist, figname, title_sentence=None, **kwargs):
        ...
        elif self.dataset_name.lower() in ["mutag", "bbbp", "bace", 'mutagenicity', 'aids']:
            x = kwargs.get("x")
            self.plot_molecule(
    ...
    def plot_subgraph(
        ...
        # Draw a circle around the center node
        center_node = [n for n in graph.nodes() if graph.nodes[n]["center"]]
        if center_node is not None:
            nx.draw_networkx_nodes(
                graph,
                pos,
                nodelist=[center for center in center_node],
                node_color="none",
                node_shape="o",
                node_size=800,
    ...
    def plot_molecule(
        ...
        if self.dataset_name == "mutag":
            ...
            node_labels = {k: atoms_aids[v] for k, v in node_idxs.items()}
            node_color = [
                "#E49D1C",  # C
                "#4970C6",
                "#FF5357",
...
def subgraph(subset, edge_index, edge_attr=None, relabel_nodes=False,
             num_nodes=None):
    r"""
    COPIED FROM TORCH GEOMETRIC TO ADAPT IT TO OUR NEEDS
